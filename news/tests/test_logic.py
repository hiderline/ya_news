...
# Для тестирования отправки комментариев и запрета стоп-слов
# в фикстурах нужно создать новость и одного авторизованного пользователя
# (для неавторизованного применим клиент,
# который в тесте создаётся автоматически).
    @classmethod
...
# ПЕРВЫЙ тест: убедимся, что анонимный пользователь не может создать
# комментарий. Для этого достаточно проверить, что при отправке
# POST-запроса на URL /news/<pk>/ в системе не появилось новых комментариев.
# При запуске тестов создаётся пустая база данных, так что проверим,
# что после отправки запроса число объектов модели Comment = 0.
...
# ВТОРОЙ тест: проверим, что залогиненный пользователь может оставить
# комментарий; отправим POST-запрос через авторизованный клиент.
# Затем посчитаем кол-во комментариев: база изначально была пустая,
# так что после добавления должно стать = 1
# Затем проверим, что поля комментария содержат корректную информацию
# В пустую БД был добавлен лишь 1 коммент, так что получить его можно
# методом objects.get(), применять .first() | .last() необязательно
...
# Пользователь не может использовать запрещённые слова при комментировании.
# Заодно проверим, что при обнаружении стоп-слов форма возвращает ошибку.
# Это можно сделать при помощи метода assertFormError()
...
# Для проверки доступа к редактированию и удалению комментариев
# понадобятся предварительно созданная новость,
# комментарий к ней и два авторизованных пользователя.
    @classmethod
    def setUpTestData(cls):
...
# При попытке отправить запрос на удаление чужого комментария
...
# Аналогичным образом проверим,
# что редактировать комментарии может только их автор.
...
# Далее проверим, что редактирование комментария недоступно
